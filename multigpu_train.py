...
from local.torch_xvector.train_utils import nnet3EgsDLNonIterable
...
def setup(rank, world_size, batch_size, args):
    dist.init_process_group(backend='nccl', init_method='env://', rank=rank, world_size=world_size)
    model = xvecTDNN(numSpkrs=7323, p_dropout=0, rank=rank).to(rank)
...
def get_egs_dataloader(egs_filepath, batch_size, world_size, rank):
    train_dataset = nnet3EgsDLNonIterable(egs_filepath)
    train_sampler = DistributedSampler(train_dataset, num_replicas=world_size, rank=rank)
    train_loader = DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=False, num_workers=0, pin_memory=True, sampler=train_sampler)
    return train_loader

def cleanup():
    dist.destroy_process_group()

def train(rank, world_size, batch_size, args):
    if rank == 0:
        checkpoint_dir = './models/{}_event_{}'.format("xvector", datetime.now().strftime('%Y%m%dT%H%M%S'))
        os.makedirs(checkpoint_dir)
    
    model, loss_fn, optimizer, lr_scheduler = setup(rank, world_size, batch_size, args)
    ...
    while step < total_steps:
        ...
        dataloader = get_egs_dataloader("{}/egs.{}.ark".format(args.featDir, archive_id), batch_size, world_size, rank)
        current_batch = 0
...
if __name__ == "__main__":
    ...
    torch.manual_seed(0)
    np.random.seed(0)
    mp.spawn(train, args=(num_gpus, 64, args), nprocs=num_gpus, join=True)
